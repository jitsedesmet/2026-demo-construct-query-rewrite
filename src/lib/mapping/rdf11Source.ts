 ...
 *    `!isTRIPLE(?x)` for each variable that could otherwise bind a triple term. The package reads
 *    `isTRIPLE`/`!isTRIPLE` as an assertion on a variable's term type, so its own assertion pushdown makes
 *    exactly the deduction the narrowed range used to make, at exactly the same point of the pipeline.
 ...
 * would replace this file with one argument, and would also cover the two corners it cannot reach - a
 * mapping body that writes a triple term into a pattern itself, and telling our own `!isTRIPLE` conjuncts
 * apart from a user query's after the pushdown has rewritten both.
 */
...
/**
 * Whether a pattern asks the source for a triple term outright, which no RDF 1.1 source holds.
 *
 * A mapping body is not supposed to write one - it matches the data, which is RDF 1.1 - but nothing stops
 * an author from doing it, and a pattern that does matches nothing at all.
 * @param pattern - The pattern to read
 * @returns whether a triple term is written anywhere in it
 */
function writesTripleTerm(pattern: Algebra.Pattern): boolean {
  return pattern.object.termType === 'Quad';
}

/**
 * Restricts every pattern of a mapping body to what an RDF 1.1 source can answer with.
 *
 * The result is the same mapping over a body in which each BGP and each property path is filtered by the
 * assumption its variables have to satisfy - which the package's assertion pushdown then either uses to
 * prune a branch or carries to the top, where
 * {@link dropRdf11SourceAssumptionTransformation} takes it out again.
...
export function withRdf11SourceAssumption(mapping: Mapping): Mapping {
  const body = algebraUtils.mapOperation<'unsafe', Algebra.Project>(mapping.body, {
    [Algebra.Types.BGP]: { transform: (bgp) => {
      ...
      if (bgp.patterns.some(writesTripleTerm)) {
...
export function dropRdf11SourceAssumptionTransformation(): QueryTransformation {
  return (_context, operation) => algebraUtils.mapOperation<'unsafe', Algebra.Operation>(operation, {
    [Algebra.Types.FILTER]: { transform: (filter) => {
      ...
      const kept = conjuncts.filter(conjunct => !isRdf11SourceAssumption(conjunct));
